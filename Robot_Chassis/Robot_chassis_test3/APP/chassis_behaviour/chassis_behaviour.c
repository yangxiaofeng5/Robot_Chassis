...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    //判断是否要摇摆
    if (chassis_move_rc_to_vector->chassis_RC->keyBoard.key_code& SWING_KEY)
    {
        if (swing_flag == 0)
        {
            swing_flag = 1;
    ...
    if (chassis_move_rc_to_vector->chassis_RC->keyBoard.key_code & CHASSIS_FRONT_KEY || chassis_move_rc_to_vector->chassis_RC->keyBoard.key_code & CHASSIS_BACK_KEY ||
        chassis_move_rc_to_vector->chassis_RC->keyBoard.key_code & CHASSIS_LEFT_KEY || chassis_move_rc_to_vector->chassis_RC->keyBoard.key_code & CHASSIS_RIGHT_KEY)
    {
...
static void chassis_engineer_follow_chassis_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    *angle_set = rad_format(chassis_move_rc_to_vector->chassis_yaw_set - CHASSIS_ANGLE_Z_RC_SEN * chassis_move_rc_to_vector->chassis_RC->ch3);
}
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
    *wz_set = -CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->ch3;
}
...
static void chassis_open_set_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    *wz_set = -chassis_move_rc_to_vector->chassis_RC->ch3 * CHASSIS_OPEN_RC_SCALE;
    return;
}
