...
void Gimbal_Task(void const * argument)
{
	...
	for(;;)
	{
		...
		wave_form_data[0] = gimbal_control.gimbal_pitch_motor.relative_angle*100;
		wave_form_data[1] = gimbal_control.gimbal_pitch_motor.relative_angle_set*100;
		
		wave_form_data[2] = gimbal_control.gimbal_yaw_motor.relative_angle*100;
		wave_form_data[3] = imu.pit;
		wave_form_data[4] = imu.rol;
		wave_form_data[5] = imu.wx;
		wave_form_data[6] = gimbal_control.gimbal_pitch_motor.given_current*10;
...
//		static int16_t yaw_channel,pitch_channel;
    ...
    rc_deadline_limit(gimbal_rc_set_control->gimbal_rc_ctrl->ch4, gimbal_rc_set_control->gimbal_value_limit.pitch_channel, RC_Dead_Band);
		...
		}else if(switch_is_up(gimbal_rc_set_control->gimbal_rc_ctrl->switch_left))
		{
			gimbal_rc_set_control->Gimbal_Motor_mode = GIMBAL_ZERO_FORCE;
		}
		...
		{
			gimbal_rc_set_control->gimbal_pitch_motor.rc_effective_stroke = gimbal_rc_set_control->gimbal_value_limit.pitch_channel*RC_TRAVEL_FACTOR;
			gimbal_rc_set_control->gimbal_yaw_motor.rc_effective_stroke = gimbal_rc_set_control->gimbal_value_limit.yaw_channel*RC_TRAVEL_FACTOR*NEGATION_COEFFICIENT;
		}
...
static void GIMBAL_YAW_MOTOR_CONTROL(Gimbal_Control_t *gimbal_yaw_motor_control)
{
		...
		if(gimbal_yaw_motor_control->gimbal_rc_ctrl->ch3 == 0)
		{
			...
			{
				gimbal_yaw_motor_control->gimbal_yaw_motor.relative_angle_set = gimbal_yaw_motor_control->gimbal_yaw_motor.relative_angle;
//					*(gimbal_yaw_motor_control->gimbal_INT_angle_point + INS_YAW_ADDRESS_OFFSET);//imu.yaw
				yaw_flag_status = 1;
			}
...
//									*(gimbal_yaw_motor_control->gimbal_INT_angle_point + INS_YAW_ADDRESS_OFFSET));
		}
		else
		{
			yaw_flag_status = 0;
...
//				fp32_constrain(gimbal_pitch_motor_control->gimbal_pitch_motor.relative_angle_set,
//											 gimbal_pitch_motor_control->gimbal_pitch_motor.min_relative_angle, 
//											 gimbal_pitch_motor_control->gimbal_pitch_motor.max_relative_angle);
			gimbal_pitch_motor_control->gimbal_pitch_motor.given_current = 
				pid_calc(&gimbal_pitch_motor_control->gimbal_pitch_motor.gimbal_motor_relative_angle_pid,
									gimbal_pitch_motor_control->gimbal_pitch_motor.relative_angle_set,
									rad_format(*(gimbal_pitch_motor_control->gimbal_INT_angle_point+INS_ROLL_ADDRESS_OFFSET)));
		}
		else
		{
			pit_flag_status = 0;
		...
		gimbal_pitch_motor_control->gimbal_pitch_motor.given_current = 
			pid_calc(&gimbal_pitch_motor_control->gimbal_pitch_motor.gimbal_motor_gyro_pid,
								...
								*(gimbal_pitch_motor_control->gimbal_INT_gyro_point+INS_GYRO_X_ADDRESS_OFFSET));//imu.wx
}
/**********************************************************************************************************
...
{
	#if PITCH_TURN
	Pitch_Set_Current 	=  -gimbal_set_motor->gimbal_pitch_motor.given_current;
	#else
	Pitch_Set_Current 	=  gimbal_set_motor->gimbal_pitch_motor.given_current;
	#endif
	
	#if YAW_TURN
	Yaw_Set_Current 	= -gimbal_set_motor->gimbal_yaw_motor.given_current;
	#else
	Yaw_Set_Current 	= gimbal_set_motor->gimbal_yaw_motor.given_current;
	#endif
